...
def tf_dropout_regression(regression, ttn, xscale, yscale, x1, y1, ntrees, filename,
                          ...
                          regression_cost_function="MSE",
                          bayesian_cost_num_samples=10
                          ):
    ...
    Args:
        ...
        random_state (int): Sets random seed.
        regression_cost_function (str): choose between 'standard' MSE and uncertainty informed 'Bayesian'
        bayesian_cost_num_samples (int): when regression_cost_function="Bayesian", take this number of samples to calculate mean and std of prediction
    ...
    for keys in ttn["weights"]:
        ...
        elif regression_cost_function == "MSE":
            cost = tf.add(tf.reduce_mean(tf.square(y-predictions)),
                      weight_decay, name="cost")
        elif regression_cost_function == "Bayesian":
            # this needs updating to avoid creating multiple applications of
            # the model
            x_stacked = tf.tile(tf.expand_dims(x, axis=1), [1, bayesian_cost_num_samples, 1])
            x_stacked = tf.reshape(x_stacked, [-1, n_input])

            pred = multilayer_perceptron(x_stacked, weights, biases)

            pred = tf.reshape(pred, [-1, bayesian_cost_num_samples, n_classes])

            y_mu = tf.reduce_mean(pred, axis=1)
            y_var = tf.clip_by_value(
                        tf.reduce_mean(tf.square(pred), axis=1)-tf.square(y_mu),
                        1.e-10,
                        1.e10
                        )

            t1 = tf.reduce_mean(tf.square(y-y_mu)/y_var)
            t2 = tf.reduce_mean(tf.math.log(y_var))

            cost = tf.add(tf.add(t1, t2), weight_decay, name="cost")
